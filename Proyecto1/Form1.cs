namespace Proyecto1
{
    public partial class Form1 : Form
    {
        ...
        private void btn_elevar_Click(object sender, EventArgs e)
        {
            ...
            // Si hay una operación pendiente, actualizamos valor2
        ...
        private void ProcesarOperacion(int tipoOperacion, string simbolo)
        {
            ...
            // Si ya había una operación pendiente, la resolvemos antes
        ...
        {
            if (txt_pantalla.Text == "" && valor2 == 0) return; // nada que hacer

            // Solo agregamos txt_pantalla.Text si es una nueva operación
            if (!nuevaOperacion)
                valor2 = double.Parse(txt_pantalla.Text);

            // Realizamos la operación pendiente
            RealizarOperacion();

            // Mostramos el resultado
            txt_pantalla.Text = resultado.ToString();

            // Actualizamos txt_ref solo si no se había marcado nuevaOperacion
            if (!nuevaOperacion)
                txt_ref.Text += valor2.ToString() + "=";

            // Preparamos para nueva operación
            valor1 = resultado;
            valor2 = 0;
            operacion = 0;
            nuevaOperacion = true;
